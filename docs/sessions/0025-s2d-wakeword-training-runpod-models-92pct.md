...
**Commits:** `50eec00`…`147e7c6` + close (kit Kaggle, guia RunPod, modelos versionados, docs de aprendizado, fechamento)
...
Entrada: sessão 0024 deixou a voz FOREGROUND (SFSpeech) funcionando e o ADR-0023 (wake-word on-device própria, ONNX) como caminho p/ background. O modelo "Raro" ainda não existia — era o gate da S2.D. Kit de treino turnkey (`livekit-wakeword` + VoxCPM) já commitado; faltava rodar numa GPU CUDA (não roda no Mac).
...
- **Lote cheio (n_samples=15000)** — tentado, **cancelado aos 60%** (9074/15000): a geração VoxCPM desacelerou (~1→3-4s/clip), ETA explodiu p/ +8h e o saldo RunPod (US$3,69 @ $0,79/h) acabaria antes do fim → seria cortado no meio. Como a prova já passou o gate (~92%), o dono optou por usar os modelos de prova e validar no iPhone primeiro. Pod terminado (custo estancado, saldo US$3,61). Plano B se o iPhone reprovar.
- **Integração iOS (`WakeWordDetector.swift`)** — NÃO iniciada. É trabalho denso (Swift nativo: carregar .onnx → áudio → inferência → disparar gravar/parar) e o dono pediu "só monitorar" nesta sessão. Fica p/ S2.D parte 2, com brainstorm → plano → TDD.
- **Validação no device** — não feita (depende da integração acima não existir ainda).
- **Débito herdado (não tocado):** gate §10 ffprobe da Frente A (pré-roll combinado — provar DTS monotônico + áudio ~0s no `vault/<id>.mp4`).

## Próximos passos

- ✅ 2 modelos ONNX (~92%) versionados, válidos, com README de proveniência.
- ✅ Aprendizados em 3 memórias + guia `COMO-TREINAR-runpod-4090.md` (com os 3 fixes) + kit Kaggle.
- ⏭️ **Próxima sessão (S2.D parte 2): integração iOS** — `WakeWordDetector.swift` (carregar o .onnx → `AudioSessionCoordinator` → inferência → disparar gravar/parar). NÃO existe ainda. onnxruntime já integrado (SPM). Fazer com brainstorm → plano → TDD. Gate: detecção "Raro" >80% no iPhone 12 + coexistência voz↔gravação. Prompt pronto em `NEXT-SESSION-PROMPT-voice-openwakeword.md`. Se device reprovar → Plano B (lote cheio / +voice_design_prompts 50-100).

## Referências

- Modelos + métricas + proveniência: `docs/superpowers/notebooks/modelos-treinados/` (README.md, raro_gravar.onnx, raro_parar.onnx, *_eval.json)
- Guia turnkey GPU paga: `docs/superpowers/notebooks/COMO-TREINAR-runpod-4090.md`
- Memórias: `raro-pattern-kaggle-p100-incompatible-force-t4`, `raro-pattern-wakeword-train-runpod-5090-hf-token`, `feedback_long_job_monitoring_measure_dont_assume`
- Prompt próxima sessão: `docs/sessions/NEXT-SESSION-PROMPT-voice-openwakeword.md`
- Sessão anterior (voz foreground SFSpeech): `0024-s2c-voice-sfspeech-works-recycle-fix-deadend-refuted.md`

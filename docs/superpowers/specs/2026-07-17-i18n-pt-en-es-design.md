...
1. **Infra:** `l10n.yaml` com `synthetic-package: false` (padrão 2026 — memória `raro-pattern-flutter-i18n-synthetic-package-false`; código gerado navegável em `lib/l10n/`), `template-arb-file: app_pt.arb`. Arquivos: `app_pt.arb` (template, com `@` descriptions), `app_en.arb`, `app_es.arb`. ICU plural/select onde couber (ex.: segundos de buffer).

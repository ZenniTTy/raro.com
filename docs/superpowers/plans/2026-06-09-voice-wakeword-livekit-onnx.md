...
6. **Treino exige GPU CUDA + VoxCPM** (PT-BR EXIGE `tts_backend: voxcpm`; **Piper é english-only** — fonte primária livekit-wakeword 0.2.1). Venue: Kaggle T4/P100 16GB (grátis, mas VoxCPM lento — cap 9h) OU RunPod/Vast 4090 (~US$1-4, mais rápido). **NÃO Mac.** Acurácia multilíngue menor → `voice_design_prompts` 50-100 + `n_samples` alto. **Kit turnkey pronto:** `docs/superpowers/notebooks/{configs/raro_gravar.yaml,configs/raro_parar.yaml,train-raro.sh}` + runbook em `raro-model-training.md`.
